...
todos:
  - id: align-terminology
    content: Pick canonical workflow phrase (match CP3) and update `.mdc` + `docs/engineering-constitution.md` consistently
    status: pending
  - id: link-cp-kpi
    content: Augment `.mdc` with explicit CP1–CP6 + primary/guardrail KPI pointer to profitability framework
    status: pending
  - id: define-major-waiver
    content: Add rubric for major work and documented test-waiver approver path
    status: pending
  - id: optional-privacy-ai
    content: Add concise data lifecycle + agent/PII logging constraints if legally relevant to leads/real estate
    status: pending
  - id: gates-ci
    content: Tie WCAG/security claims to concrete PR/CI checks (`checks:ci`, lint, secret scan) in constitution or PR template
    status: pending
isProject: false
---
